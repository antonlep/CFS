...
class EigenMatrixProvider:
    def __init__(self, valobj: lldb.SBValue, internal_dict):
        ...
        try:
            self._rows_compile_time: int = (
                int(template_args[1]) if len(template_args) > 1 else -1
            )
        except (ValueError, IndexError):
            self._rows_compile_time: int = -1

        try:
            self._cols_compile_time: int = (
                int(template_args[2]) if len(template_args) > 2 else -1
            )
        except (ValueError, IndexError):
